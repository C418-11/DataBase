...
class LevelSet:
    ...
    def add(self, element: Level):
        ...
        if element.weight in self.LevelToName.keys():
            raise ValueError(f"Weight {element.weight} already exists")

        self.NameToLevel[element.name] = element.weight
        self.LevelToName[element.weight] = element.name

    def GetLevelName(self, level: Union[int, float, Level]):
        if isinstance(level, Level):
            level = level.weight
        return self.LevelToName[level]

    def GetNameLevel(self, name: Union[str, Level]):
...
def getLevelName(level: Union[int, float, Level]) -> str:
    return _levels.GetLevelName(level=level)


def getNameLevel(name: str) -> int:
    return _levels.GetNameLevel(name=name)
...
class Logger:
    ...
    def log(self, msg: str, level: Union[Level, str, int, float]):
        if self.disable:
            return
        if self.type is not str:
            raise TypeError(f"Str not {type(self.type)}")
        if type(level) is str:
            level = getNameLevel(name=level)
    ...
    def bin_log(self, msg: Union[bytes, bytearray], level: Union[Level, str, int, float]):
        if self.disable:
            return
